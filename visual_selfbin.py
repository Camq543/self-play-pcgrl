...
####### REWARD PLOT #######
d = open("logs\self_play_binary_narrow_log.txt", "r") #CHANGE THIS

s = d.read()
x = re.findall('\{(.*?)\}',s)
y = re.findall("'path-change': \[(-?.*?\])",s) #specify feature name here
num = str(y)[1:-1]
...
plt.ylabel('Path Change') #CHANGE THIS
...
####### KEY PLOT #######
h = open("logs\self_play_binary_narrow_negative_switch_log.txt", "r") #CHANGE THIS
...
####### DOOR PLOT #######
u = open("logs\self_play_binary_turtle_log.txt", "r") #CHANGE THIS

w = u.read()
x = re.findall('\{(.*?)\}',w)
y = re.findall("'path-change': \[(-?.*?\])",w) #specify feature name here
num = str(y)[1:-1]
...
####### DOOR PLOT #######
j = open("logs\self_play_binary_turtle_negative_switch_log.txt", "r") #CHANGE THIS
...
plt.title("Self Play - Binary") #CHANGE THIS
plt.legend(loc="lower left")
plt.xlabel('Update Interval')
plt.ylabel('Path Change Values') #CHANGE THIS

strFile = "./figs/self_play_binary_pathchange.png" #CHANGE THIS
if os.path.isfile(strFile):
  os.remove(strFile)
plt.savefig(strFile)
plt.show()
